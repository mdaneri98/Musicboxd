...
export const UserInfo: React.FC<UserInfoProps> = ({ user, isOwnProfile, isAuthenticated, isFollowing, followLoading, onFollowToggle }) => {
  ...
  const renderActionButton = () => {
    ...
    if (isOwnProfile) {
      return (
        <Link href="/profile/edit" className="btn btn-primary btn-lg">
          {t("label.editProfile")}
        </Link>
      );
    }
    ...
    return (
      <button 
        className={`btn btn-lg ${isFollowing ? 'btn-secondary' : 'btn-primary'}`} 
    ...
    <section className="user-profile-header">
      <div className="user-profile-main">
        ...
        <div className="user-profile-info-wrapper">
          {/* Header row with entity type and action button */}
