        ...
        if (filter === 'all' || filter === 'users') {
            searchPromises.push(
                userRepository.getUsers(0, 10, query).then((response) => {
                    results.users = response.items.map((item) => item.data);
                })
            );
        }
        ...
        if (filter === 'all' || filter === 'artists') {
            searchPromises.push(
                artistRepository.getArtists(0, 10, query).then((response) => {
                    results.artists = response.items.map((item) => item.data);
                })
            );
        }
        ...
        if (filter === 'all' || filter === 'albums') {
            searchPromises.push(
                albumRepository.getAlbums(0, 10, query).then((response) => {
                    results.albums = response.items.map((item) => item.data);
                })
            );
        }
        ...
        if (filter === 'all' || filter === 'songs') {
            searchPromises.push(
                songRepository.getSongs(0, 10, query).then((response) => {
                    results.songs = response.items.map((item) => item.data);
                })
            );
        }
        ...
        if (filter === 'all' || filter === 'reviews') {
            searchPromises.push(
                reviewRepository.getReviews(0, 10, query).then((response) => {
                    results.reviews = response.items.map((item) => item.data);
                })
            );
        }
